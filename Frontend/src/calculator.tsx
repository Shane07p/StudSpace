...
import API from './api';
import { Icon, Card, Button, useCountUp } from './lib';
import type { CourseDto } from './types';

interface CalcState {
  currentCGPA: number;
  creditsEarned: number;
  currentSemCredits: number;
  targetCGPA: number;
...
type GradeLetter = typeof GRADES[number]['letter'];

function calcRequiredSGPA({ currentCGPA, creditsEarned, currentSemCredits, targetCGPA }: CalcState): number | null {
  if (currentSemCredits <= 0) return null;
  return (targetCGPA * (creditsEarned + currentSemCredits) - currentCGPA * creditsEarned) / currentSemCredits;
}

function classifyGoal(req: number | null) {
...
function RequiredSGPACard({ state, setState }: { state: CalcState; setState: React.Dispatch<React.SetStateAction<CalcState>> }) {
  ...
  const toneClass: Record<string, string> = { good: "text-emerald-600 dark:text-emerald-400", warn: "text-amber-600 dark:text-amber-400", bad: "text-rose-600 dark:text-rose-400", neutral: "text-neutral-700 dark:text-neutral-200" };
  const ringColor: Record<string, string> = { good: "#10b981", warn: "#f59e0b", bad: "#f43f5e", neutral: "#6366f1" };

  return (
    <Card className="p-6">
      <div className="flex items-center gap-2 text-[11.5px] font-medium uppercase tracking-wider text-[var(--accent)]"><Icon name="Target" size={12} /> Required SGPA</div>
      <h3 className="mt-2 text-[16px] font-semibold tracking-tight text-neutral-900 dark:text-neutral-50">What do I need this semester to reach my target CGPA?</h3>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
        <NumField label="Current CGPA"           value={state.currentCGPA}        onChange={(v) => setState(s => ({ ...s, currentCGPA: clamp(v, 0, 10) }))}       min={0} max={10}  step={0.01} decimals={2} hint="On a 10-point scale" />
        <NumField label="Credits earned so far"  value={state.creditsEarned}      onChange={(v) => setState(s => ({ ...s, creditsEarned: clamp(v, 0, 250) }))}    min={0} max={250} step={1}                hint="Across all prior semesters" />
        <NumField label="This semester's credits" value={state.currentSemCredits} onChange={(v) => setState(s => ({ ...s, currentSemCredits: clamp(v, 1, 40) }))} min={1} max={40}  step={1}                hint="Sum of all enrolled courses" />
        <NumField label="Target CGPA"            value={state.targetCGPA}         onChange={(v) => setState(s => ({ ...s, targetCGPA: clamp(v, 0, 10) }))}        min={0} max={10}  step={0.01} decimals={2} hint="What you want to end this semester at" />
      </div>
        ...
        <div className="flex items-center gap-5">
          <div className="relative">
            <svg width="92" height="92" viewBox="0 0 92 92" className="-rotate-90">
              ...
              <circle cx="46" cy="46" r="38" stroke={ringColor[verdict.tone]} strokeWidth="6" fill="none" strokeLinecap="round"
            ...
            <div className="absolute inset-0 grid place-items-center text-center">
              <div>
                <div className={`text-[24px] font-semibold tabular-nums tracking-tight leading-none ${toneClass[verdict.tone]}`}>
          ...
          <div className="flex-1">
            <div className={`text-[12px] font-semibold uppercase tracking-wider ${toneClass[verdict.tone]}`}>{verdict.label}</div>
            <div className="mt-1 text-[13.5px] text-neutral-700 dark:text-neutral-200 leading-snug">
              {req == null ? "Enter your this-semester credits to calculate."
                : req <= 0 ? <>You'll reach <span className="font-semibold tabular-nums">{state.targetCGPA.toFixed(2)}</span> even with a 0 this semester. Coast or aim higher.</>
                : req > 10 ? <>This target isn't reachable in one semester. Reduce target to <span className="font-semibold tabular-nums">{Math.min(10, projected).toFixed(2)}</span> or earn more credits.</>
              ...
              {GRADES.map(g => {
                ...
                return (
                  <div key={g.letter} className={`flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-medium ${hits ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "bg-neutral-100 dark:bg-white/[0.04] text-neutral-500 dark:text-neutral-400"}`}>
                    <span className="font-mono">{g.letter}</span><span className="text-neutral-400 dark:text-neutral-500">·</span><span className="tabular-nums">{g.points}</span>
...
function PlannerCard({ state, courses }: { state: CalcState; courses: CourseDto[] }) {
  const [plan, setPlan] = React.useState<Record<string, GradeLetter>>({});

  React.useEffect(() => {
    setPlan(Object.fromEntries(courses.map(c => [c.id, "A+"])) as Record<string, GradeLetter>);
  }, [courses]);

  const setGrade = (id: string, letter: GradeLetter) => setPlan(p => ({ ...p, [id]: letter }));
  ...
  return (
    <Card className="p-6">
      <div className="flex items-center gap-2 text-[11.5px] font-medium uppercase tracking-wider text-[var(--accent)]"><Icon name="Sparkles" size={12} /> What-if planner</div>
      <h3 className="mt-2 text-[16px] font-semibold tracking-tight text-neutral-900 dark:text-neutral-50">Plan grades by course</h3>
      <p className="mt-1 text-[12.5px] text-neutral-500 dark:text-neutral-400">Pick a realistic grade for each course. We'll show your projected SGPA and updated CGPA.</p>
      {courses.length === 0 ? (
        <div className="mt-5 rounded-lg border border-dashed border-neutral-200 dark:border-white/[0.08] py-8 text-center text-[13px] text-neutral-400">No courses in current semester. Add them in the Semesters page.</div>
      ) : (
        <>
          <div className="mt-5 flex flex-col gap-1.5">
            {courses.map((c) => (
              <div key={c.id} className="grid grid-cols-[88px_1fr_auto] items-center gap-3 rounded-md px-2 py-2 hover:bg-neutral-50 dark:hover:bg-white/[0.03] transition-colors">
                ...
                <div className="min-w-0"><div className="truncate text-[13px] font-medium text-neutral-800 dark:text-neutral-100">{c.name}</div><div className="text-[11px] text-neutral-500 dark:text-neutral-400 tabular-nums">{c.credits} credits</div></div>
...
function GradePicker({ value, onChange }: { value: string; onChange: (g: GradeLetter) => void }) {
  return (
    <div className="inline-flex items-center gap-0.5 rounded-md border border-neutral-200/80 dark:border-white/[0.08] bg-neutral-50 dark:bg-white/[0.03] p-0.5">
      {GRADES.map(g => {
        ...
        return (
          <button key={g.letter} onClick={() => onChange(g.letter)} className={`min-w-[28px] px-1.5 py-1 rounded-[5px] text-[11px] font-mono font-medium transition-colors ${active ? "bg-white text-neutral-900 shadow-sm dark:bg-white/[0.1] dark:text-white" : "text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200"}`} title={`${g.letter} = ${g.points}`}>
            {g.letter}
          </button>
        );
      })}
...
function CalcStat({ label, value, hint, tone = "neutral" }: { label: string; value: string; hint: string; tone?: string }) {
...
function clamp(n: number, min: number, max: number): number { return Math.max(min, Math.min(max, isNaN(n) ? min : n)); }

interface NumFieldProps { label: string; hint?: string; value: number; onChange: (v: number) => void; min: number; max: number; step: number; decimals?: number; }

function NumField({ label, hint, value, onChange, min, max, step, decimals = 0 }: NumFieldProps) {
  const [raw, setRaw] = React.useState("");
  const [focused, setFocused] = React.useState(false);
  const display = decimals ? Number(value).toFixed(decimals) : String(value);
  const commit = (str: string) => { const n = parseFloat(str); if (!isNaN(n)) onChange(clamp(n, min, max)); };
  return (
    <label className="flex flex-col gap-1.5">
      <span className="text-[11.5px] font-medium uppercase tracking-wider text-neutral-500 dark:text-neutral-400">{label}</span>
      <input type="text" inputMode="decimal" value={focused ? raw : display}
...
function CalcSummaryStat({ icon, label, value, sub, accent }: { icon: string; label: string; value: string; sub?: string; accent?: boolean }) {
  return (
    <Card className="p-4">
      <div className="flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wider text-neutral-500 dark:text-neutral-400"><Icon name={icon} size={11} /><span>{label}</span></div>
...
function CalculatorPage() {
  const [state, setState] = React.useState<CalcState>(DEFAULTS);
  ...
  return (
    <div className="mx-auto w-full max-w-[1240px] px-6 py-7 lg:px-10 lg:py-9 page-fade">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="flex items-center gap-2 text-[12px] text-neutral-500 dark:text-neutral-400"><Icon name="Calculator" size={12} /><span>Calculator</span></div>
        ...
        <Button variant="outline" onClick={() => setState(DEFAULTS)}><Icon name="RotateCcw" size={12} /> Reset</Button>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        <PlannerCard state={state} courses={courses} />
      </div>
      <Card className="mt-6 p-5 bg-neutral-50/60 dark:bg-white/[0.02]">
        <div className="flex items-start gap-3">
          <div className="grid h-7 w-7 place-items-center rounded-md bg-[var(--accent)]/10 text-[var(--accent)] shrink-0"><Icon name="Info" size={14} /></div>
          <div className="text-[12px] text-neutral-600 dark:text-neutral-400 leading-relaxed">
            ...
            <div className="mt-1 text-pretty">
              <span className="font-mono text-[11px] bg-neutral-100 dark:bg-white/[0.04] px-1.5 py-0.5 rounded">SGPA = Σ(credits × grade points) / Σ(credits)</span>{" "}for a single semester.{" "}
              <span className="font-mono text-[11px] bg-neutral-100 dark:bg-white/[0.04] px-1.5 py-0.5 rounded">CGPA = (CGPA_prev × credits_prev + SGPA × credits_this) / (credits_prev + credits_this)</span>{" "}for the running average. Grade points use the 10-point scale (O=10, A+=9, A=8, B+=7, B=6, C=5, P=4).
            </div>
          </div>
        </div>
      </Card>
