...
export function useTweaks<T extends object>(defaults: T): [T, (keyOrEdits: keyof T | Partial<T>, val?: T[keyof T]) => void] {
...
export function TweakRadio({ label, value, options, onChange }: { label: string; value: string; options: (string | TweakOption)[]; onChange: (v: string) => void }) {
  const trackRef = React.useRef<HTMLDivElement>(null);
  const [dragging, setDragging] = React.useState(false);
  const valueRef = React.useRef(value);
  valueRef.current = value;

  const labelLen = (o: string | TweakOption) => String(typeof o === 'object' ? o.label : o).length;
  const maxLen = options.reduce((m, o) => Math.max(m, labelLen(o)), 0);
  const lengthCap: Record<number, number | undefined> = { 2: 16, 3: 10 };
  const fitsAsSegments = maxLen <= (lengthCap[options.length] ?? 0);
  if (!fitsAsSegments) {
...
export function TweakSelect({ label, value, options, onChange }: { label: string; value: string; options: (string | TweakOption)[]; onChange: (v: string) => void }) {
  return (
    <TweakRow label={label}>
...
export function TweakColor({ label, value, options, onChange }: { label: string; value: string; options?: (string | string[])[]; onChange: (v: string | string[]) => void }) {
    ...
    <TweakRow label={label}>
      <div className="twk-chips" role="radiogroup">
        {options.map((o, i) => {
          ...
          const on = key(o) === cur;
          return (
            <button key={i} type="button" className="twk-chip" role="radio" aria-checked={on} data-on={on ? '1' : '0'} aria-label={colors.join(', ')} title={colors.join(' · ')} style={{ background: hero }} onClick={() => onChange(o)}>
