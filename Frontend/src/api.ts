import type {
  UserProfileDto, HandleDto, SemesterDto, CourseDto,
  ResourceDto, AttendanceDto, TimetableSlotDto,
  DashboardResponse, AttendanceStatus, ResourceType,
} from './types';
...
function redirectToDashboard(): void { window.location.href = 'dashboard.html'; }
...
interface AuthResponse { token: string; user: UserProfileDto; }
...
const API = {
  ...
  auth: {
    ...
    register: async (fullName: string, username: string, email: string, password: string): Promise<AuthResponse> => {
  ...
  attendance: {
    get: (courseId: string): Promise<unknown> => request('GET', '/courses/' + courseId + '/attendance'),
    upsert: (courseId: string, date: string, status: AttendanceStatus): Promise<unknown> =>
